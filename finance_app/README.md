...
停止工作台可在啟動它的終端機按 `Ctrl+C`，或在另一個終端機執行：

```sh
./stop.sh
```

若啟動時使用其他連接埠，請指定相同連接埠，例如 `./stop.sh 8502`（也支援 `./stop.sh --server.port 8502`）。停止腳本會確認程序的專案目錄與啟動指令，只對此專案的工作台送出正常終止訊號。
...
換電腦或重新下載專案時，請在新電腦重做上面的環境安裝步驟；`.venv/` 不隨 Git 同步，也不應直接沿用另一台電腦的虛擬環境。若要延續原本的自選與設定，請另行將原電腦的 `data/preferences.json` 複製到新專案的相同位置；未提供時會使用預設設定。
