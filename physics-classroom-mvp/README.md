...
更新：２０２６－０９－１７。Three.js ＋ Firebase Hosting ＋ Realtime Database ＋ Firebase Authentication。
...
- 向量運算另支援投影與夾角，零向量無定義時顯示提示。
- 拋體在 XZ 平面、Z 向上，可設定高度、初速率、仰角、重力與質量；落地停止，忽略空氣阻力。
- 簡諧運動沿 X 軸，可設定振幅、角頻率、初相位與質量；顯示速度、加速度、力與能量。
- 自由向量可啟用三軸拖曳，放開後同步；QR code 用於學生加入。
- 場景庫儲存在當前瀏覽器；跨電腦請匯出／匯入 JSON。

首頁先驗證老師密碼及資料庫權限，才進入老師端。密碼不放入網址。若曾使用舊版首頁登入，建議本人更換老師密碼，因舊網址可能已留在歷史紀錄中。此次更新無法撤回舊紀錄。
