...
function finishTurn() {
  ...
  if (solved && moveQueue.length === 0) stopSolveTimer();
...
function startSolveTimer() {
  if (!solveTimeEl || solveTimerRunning) return;
  solveTimerElapsed = 0;
  solveTimerStartedAt = performance.now();
  solveTimerRunning = true;
  updateSolveTimer();
}

function stopSolveTimer() {
  if (!solveTimeEl || !solveTimerRunning) return;
  solveTimerElapsed = performance.now() - solveTimerStartedAt;
  solveTimerRunning = false;
  updateSolveTimer();
}
...
function updateSolveTimer(now = performance.now()) {
  if (!solveTimeEl) return;
  const elapsed = solveTimerRunning ? now - solveTimerStartedAt : solveTimerElapsed;
  solveTimeEl.textContent = formatSolveTime(elapsed);
}
